...
struct cpp_sdk_output : public output_format {
    struct factory {
        factory() {
            output_format::add(new cpp_sdk_output());
        }
    };

    cpp_sdk_output() : output_format("cpp-sdk") {}
    ...
    int output(std::string destination, std::vector<std::string> output_options,
        ...
        for (const auto &program : source.programs) {
            ...
            if (program.sideset_bits_including_opt.is_specified()) {
                fprintf(out, "#define %ssideset_bits %d\n", prefix.c_str(), program.sideset_bits_including_opt.get());
                fprintf(out, "#define %ssideset_opt %s\n", prefix.c_str(), program.sideset_opt ? "true" : "false");
            ...
            fprintf(out, "static constexpr uint16_t %sprogram_instructions[] = {\n", prefix.c_str());
            ...
            fprintf(out, "#if !PICO_NO_HARDWARE\n");
            fprintf(out, "static constexpr struct pio_program %sprogram = {\n", prefix.c_str());
            ...
            fprintf(out, "    sm_config_set_wrap(&c, offset + %swrap_target, offset + %swrap);\n", prefix.c_str(),
                    prefix.c_str());
            if (program.sideset_bits_including_opt.is_specified()) {
                fprintf(out, "    sm_config_set_sideset(&c,\n");
                fprintf(out, "        %ssideset_bits,\n", prefix.c_str());
                fprintf(out, "        %ssideset_opt,\n", prefix.c_str());
                fprintf(out, "        %ssideset_pindirs\n", prefix.c_str());
                fprintf(out, "    );\n");
            }
...
static cpp_sdk_output::factory creator;
